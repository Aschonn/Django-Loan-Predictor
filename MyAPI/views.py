...
def cxcontact(request):
	if request.method=='POST':
		...
		if form.is_valid():
				...
				if answer == 'Rejected':
					messages.warning(request,'Application Status: {}'.format(answer))
				elif int(df['LoanAmount'])<25000:
					messages.warning(request,'Invalid: Your Loan Request Exceeds $25,000 Limit')
				else:
...
def cxcontact2(request):
	if request.method=='POST':
		...
		if form.is_valid():
				...
				Xscalers=approvereject(ohevalue(df))[1]
				if answer == 'Rejected':
					messages.danger(request,'Application Status: {}'.format(answer))
				elif int(df['LoanAmount'])<25000:
					messages.warning(request,'Invalid: Your Loan Request Exceeds $25,000 Limit')
				else:
					messages.success(request,'Application Status: {}'.format(answer))
